...
하나의 Excel 통합문서 안에 필요한 시트를 구성합니다. 시트 이름은 아래 권장 이름을 사용하는 것이 가장 안전합니다.

| 시트 | 용도 | 주요 컬럼 |
| --- | --- | --- |
| `Sheet1` | 진단 데이터 | `DIAG_DATE`, `ICD_NAME`, `Classification1`, `Classification2`, `MDRP_NO`, `PTNO` |
| `Sheet3` | 환자 및 입·퇴원 데이터 | `ADMTIME`, `DISCHTIME`, `sex`, `ADM_DEPARTMENT`, `MDRP_NO`, `PTNO`, `age` |
| `Sheet4` | 항생제 투여 데이터 | 환자번호, `ATC_Code`, 성분명, 일회량, 단위, 단위환산함량, 투여일시 또는 투여일 |
| `Sheet5` | 월별 재원일수 | `month`, `patient_days` |

`Sheet1`과 `Sheet3`은 각각 Diagnosis와 Patients 대시보드에 사용됩니다. DOT / DDD 대시보드를 사용하려면 항생제 투여 데이터인 `Sheet4`와 월별 재원일수 데이터인 `Sheet5`가 모두 필요합니다.

### 데이터 작성 시 확인사항

- 날짜는 Excel 날짜 형식 또는 `YYYY-MM-DD` 형식을 권장합니다.
- 월 정보는 `YYYY-MM` 또는 `YYYYMM` 형식을 사용할 수 있습니다.
- DDD 분석에는 항생제별 DDD 값과 단위가 정확히 입력되어야 합니다.
- DDD 화면은 성인 환자(15세 이상)를 대상으로 합니다.
- 환자, 진단, 투여 데이터에서 환자 식별 기준이 일관되어야 필터와 집계가 정확하게 연결됩니다.
- 컬럼 이름 앞뒤의 불필요한 공백과 병합 셀은 사용하지 않는 것이 좋습니다.
...
이 저장소에는 분석 프로그램 코드와 화면 리소스만 포함하며, 환자 데이터는 포함하지 않습니다.
...
- 필요한 시트 이름과 주요 컬럼이 `Excel 파일 구성`에 맞는지 확인합니다.
